...
def atom_features(atom):
    return np.array(one_of_k_encoding_unk(atom.GetSymbol(),['C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl', 'Br', 'Mg', 'Na','Ca', 'Fe', 'As', 'Al', 'I', 'B', 'V', 'K', 'Tl', 'Yb','Sb', 'Sn', 'Ag', 'Pd', 'Co', 'Se', 'Ti', 'Zn', 'H','Li', 'Ge', 'Cu', 'Au', 'Ni', 'Cd', 'In', 'Mn', 'Zr','Cr', 'Pt', 'Hg', 'Pb', 'Unknown']) +
                    ...
                    [atom.GetIsAromatic()])
# 获取元素的原子符号、相邻原子个数、相邻氢原子个数、原子的隐含值、原子是否处于芳香结构中

def one_of_k_encoding(x, allowable_set):
    if x not in allowable_set:
        raise Exception("input {0} not in allowable set{1}:".format(x, allowable_set))
    return list(map(lambda s: x == s, allowable_set))
# 返回在allowable_set中x=s的值，存在List中
...
def one_of_k_encoding_unk(x, allowable_set):
    ...
    if x not in allowable_set:
        x = allowable_set[-1]
    return list(map(lambda s: x == s, allowable_set))
# 返回在allowable_set中x=s的值，存在List中，都不同的话认为最后一个相同
...
def seq_cat(prot):
    ...
    for i, ch in enumerate(prot[:max_seq_len]):
        # enumerate遍历索引又要遍历元素，从头到max_seq_len
        x[i] = seq_dict[ch]
    return x  
...
for dataset in datasets:
    ...
    # object_pairs_hook=OrderedDict有序的键值对表
    ...
    for opt in opts:
        ...
        # np.where只有条件 (condition)，没有x和y，则输出满足条件 (即非0) 元素的坐标
        # np.isnan以元素方式测试NaN，并以布尔数组的形式返回结果
